...
if(!isset($_SESSION['admin'])){
	...
	if(isset($_GET['id'])){
		if(is_numeric($_GET['id'])){
			$id = $_GET['id'];
			$sql = mysql_query("SELECT * FROM df_morenews WHERE id = '".$id."'");
			if ($data = mysql_fetch_assoc($sql)) {
				$text = $data['text'];
				$title = $data['title'];
				$id = $data['id'];
				$message = '<form action="editmore-news.php" method="POST"><table border="0">
		...
		$sql = mysql_query("UPDATE df_news SET text = '$text', title = '$title' WHERE id = '$id'");
...
<html>
<head>
<title><?php echo $sitename; ?> - Edit More-News</title>
