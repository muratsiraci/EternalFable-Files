...
if (!isset($HTTP_RAW_POST_DATA)) {
    ...
    if($user != 1 || $error == 1){
        ...
        {
            $reason = "Error!";
            $message = "Could not find item in database: ".$item_id.".";
            ...
            $query = mysql_query("SELECT * FROM df_equipment WHERE ItemID = '".$item['ItemID']."' AND CharID = '".$char_id."'") or $error = 1;
            $query_rows = mysql_num_rows($query);

            if($query_rows > 0) {
                while($query_fetched = mysql_fetch_assoc($query)){
                    //ADD 1 TO BANK & DELETE FROM INVENTORY
                    $additem = mysql_query("INSERT INTO `df_bank` (`id`, `CharID`, `ItemID`, `Level`, `Exp` ) VALUES ('', '".$char_id."', '".$item_id."', '".$query_fetched['Level']."', '".$query_fetched['Exp']."')") or $error = 1;
                    $removeitem = mysql_query("DELETE FROM `df_equipment` WHERE `CharID` = ".$char_id." AND `ItemID` = ".$item_id." LIMIT 1") or $error = 1;
                }
            }

            $bankquery = mysql_query("SELECT * FROM df_bank WHERE CharID = '".$char_id."'") or $error = 1;
            $bank_rows = mysql_num_rows($bankquery);

            if ($error == 1)
            {
            	$reason = "Error!";
            	$message = "Error with Inventory Items.";
                ...
                if($bank_rows >= 0) {
                    while($bank = mysql_fetch_assoc($bankquery)){
                        ...
                        $items->setAttribute("ItemID", $invent["ItemID"]);
