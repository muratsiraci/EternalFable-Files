...
if (!isset($HTTP_RAW_POST_DATA)) {
    ...
    $intPVPCharID = $doc->getElementsByTagName('intPVPCharID');
    $char_id = $intPVPCharID->item(0)->nodeValue;

    $char_result = mysql_query("SELECT * FROM df_characters WHERE id != '" . $char_id . "' ORDER BY RAND() LIMIT 1") or $error = 1;
    $char = mysql_fetch_assoc($char_result);

    $user_result = mysql_query("SELECT * FROM df_users WHERE id = '".$char['userid']."' LIMIT 1") or $error = 1;
