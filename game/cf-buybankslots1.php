...
if (!isset($HTTP_RAW_POST_DATA)) {
    ...
    if($user != 1 || $error == 1){
        ...
        $info->setAttribute("action", "None");
            ...
            if ($char['MaxBankSlots'] < 10){
                $cost = $intNumSlots * 100;
            } else if ($char['MaxBankSlots'] >= 10 && $char['MaxBankSlots'] < 15){
                $cost = $intNumSlots * 300;
            } else if ($char['MaxBankSlots'] >= 15 && $char['MaxBankSlots'] < 35){
                $cost = $intNumSlots * 500;
            }
            $newcoins = $char['Coins'] - $cost;
            if ($newcoins < 0){
                $reason = "Error!";
                ...
                $takecoins = mysql_query("UPDATE df_characters SET coins='" . $newcoins . "' WHERE ID='" . $char_id . "' LIMIT 1") or $error = 1;
                $newslots = $char['MaxBankSlots'] + $NumSlots;
                $addslots = mysql_query("UPDATE df_characters SET MaxBankSlots='" . $newslots . "' WHERE ID='" . $char_id . "' LIMIT 1") or $error = 1;
            }

        if($error == 1) {
            $reason = "Error!";
            $message = "Could not load update bank slots";
