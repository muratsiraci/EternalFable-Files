...
if (!isset($HTTP_RAW_POST_DATA)) {
    ...
    if($user != 1 || $error == 1){
        ...
        //Saves String to DB
        $savestring = mysql_query("UPDATE df_characters SET strQuests='" . $newqstr . "' WHERE ID='" . $char_id . "' LIMIT 1") or $error = 1;

        if ($error == 1)
        {
            $reason = "Error!";
            $message = "Cannot save Quest Data.";
