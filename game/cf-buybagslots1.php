...
if (!isset($HTTP_RAW_POST_DATA)) {
    ...
    if($user != 1 || $error == 1){
        ...
        if ($char['MaxBagSlots'] <= 10){
            $cost = $NumSlots * 100;
        } else if ($char['MaxBagSlots'] > 10 && $char['MaxBagSlots'] <= 15){
            $cost = $NumSlots * 300;
        } else if ($char['MaxBagSlots'] > 15 && $char['MaxBagSlots'] <= 20){
            $cost = $NumSlots * 200;
        } else if ($char['MaxBagSlots'] > 20 && $char['MaxBagSlots'] <= 25){
            $cost = $NumSlots * 500;
        }
        ...
        $newslots = $char['MaxBagSlots'] + $NumSlots;
        $addslots = mysql_query("UPDATE df_characters SET MaxBagSlots='" . $newslots . "' WHERE ID='" . $char_id . "' LIMIT 1") or $error = 1;
        if($error == 1) {
            $reason = "Error!";
