...
if (!isset($HTTP_RAW_POST_DATA)) {
    ...
    $strItems = $doc->getElementsByTagName('strItems');
    $Items = $strItems->item(0)->nodeValue;
    ...
    if($user != 1 || $error == 1){
        ...
        $query = mysql_query("UPDATE `df_equipment` SET `StartingItem` = '0' WHERE `CharID` = ".$char_id." AND `StartingItem` = '1' LIMIT 1") or $error = 1;

        $replaced = str_replace(",", " AND `CharID` = ".$char_id." ; UPDATE `df_equipment` SET `StartingItem` = '1' WHERE `ItemID` = ", $Items);
        $query2 = mysql_query("UPDATE `df_equipment` SET `StartingItem` = '1' WHERE `ItemID` = ".$replaced."") or $error = 1;

        if($error == 1){
            $reason = "Error!";
            $message = "UPDATE `df_equipment` SET `StartingItem` = '1' WHERE `ItemID` = ".$replaced."";
